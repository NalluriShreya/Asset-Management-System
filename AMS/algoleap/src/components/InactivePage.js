...
//         const res = await axios.get(`http://localhost:8080/algoleap/inactive/${username}`);
...
const InactivePage = () => {
  ...
  // const handleMakeActive = async (employee_id) => {
  //   try {
  //     const res = await axios.post('http://localhost:8080/algoleap/make_active', { employee_id });
  //     if (res.data.success) {
  //       navigate(`/algoleap/${username}`);
  //     }
  //   } catch (err) {
            ...
            <tbody>
              {userData.map((employee, index) => (
                <tr className="emp-info" key={index}>
                  ...
                  <td>{employee.reporting_manager}</td>
                  <td>{employee.status}</td>
                  {/* <td><button onClick={() => handleMakeActive(employee.employee_id)} className='make_active'>Make&nbsp;Active</button></td> */}
                </tr>
              ))}
            </tbody>
          </table>
